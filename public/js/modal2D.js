export function showModal2D(userData) {
    ...
    const destinationUrlInput = document.getElementById('destinationUrlInput').value;
    const destinationEndpointInput = document.getElementById('destinationEndpointInput').value;
    const destinationUsernameInput = document.getElementById('destinationUsernameInput').value;
    const destinationPasswordInput = document.getElementById('destinationPasswordInput').value;

    modal.style.display = 'block';

    const url = 'https://vise-backend-sleepy-leopard-to.cfapps.eu20-001.hana.ondemand.com/fetch-url';

    const basicAuth = 'Basic ' + btoa(destinationUsernameInput + ':' + destinationPasswordInput);

    let data;
    console.log(destinationUrlInput);
    ...
    fetch(url, {
        ...
        body: JSON.stringify({
            url: destinationUrlInput + "/" + destinationEndpointInput
        })
    })
